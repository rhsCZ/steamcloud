...
void CsteamuploadDlg::Clearlist()
{
...
CsteamuploadDlg::CsteamuploadDlg(CWnd* pParent /*=nullptr*/)
	: CDialog(IDD_STEAMUPLOAD_DIALOG, pParent)
{
...
void CsteamuploadDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CsteamuploadDlg, CDialog)
	ON_WM_PAINT()
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_WM_SYSCOMMAND()
	...
	ON_BN_CLICKED(IDC_TRAYEN, &CsteamuploadDlg::OnBnClickedTrayEn)
	ON_BN_CLICKED(IDC_MINEN, &CsteamuploadDlg::OnBnClickedMinEn)
	ON_BN_CLICKED(IDC_EXIT, &CsteamuploadDlg::OnBnClickedExit)
	ON_COMMAND(ID_MENU_EXIT, &CsteamuploadDlg::OnBnClickedExit)
	ON_COMMAND(ID_MENU_MAX, &CsteamuploadDlg::OnOpen)
	ON_COMMAND(ID_MENU_MIN, &CsteamuploadDlg::OnMinimize)
	ON_BN_CLICKED(IDC_CONNECT, &CsteamuploadDlg::OnBnClickedConnect)
	ON_BN_CLICKED(IDC_DELETE, &CsteamuploadDlg::OnBnClickedDelete)
	ON_BN_CLICKED(IDC_UPLOAD, &CsteamuploadDlg::OnBnClickedUpload)
	ON_BN_CLICKED(IDC_DIRUPLOAD, &CsteamuploadDlg::OnBnClickedDirupload)
	ON_BN_CLICKED(IDC_DOWNLOAD, &CsteamuploadDlg::OnBnClickedDownload)
	ON_BN_CLICKED(IDC_REFRESH, &CsteamuploadDlg::OnBnClickedRefresh)
	ON_BN_CLICKED(IDC_DISCONNECT, &CsteamuploadDlg::OnBnClickedDisconnect)
END_MESSAGE_MAP()
BOOL CsteamuploadDlg::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
BOOL CsteamuploadDlg::OnInitDialog()
{
	...
	CsteamuploadDlg::ShowWindow(SW_SHOW);
	CsteamuploadDlg::RedrawWindow();
	CsteamuploadDlg::CenterWindow();
	CDialog::OnInitDialog();
	out = RegCrtKey(HKEY_CURRENT_USER, "Software\\Steamupload", KEY_ALL_ACCESS | KEY_WOW64_64KEY);
	if (out == 1)
	{
		RegSetKey(HKEY_CURRENT_USER, "Software\\Steamupload", REG_DWORD, KEY_ALL_ACCESS | KEY_WOW64_64KEY, "TrayEnable", &indata);
		RegSetKey(HKEY_CURRENT_USER, "Software\\Steamupload", REG_DWORD, KEY_ALL_ACCESS | KEY_WOW64_64KEY, "TrayMinimize", &indata);
	}
	else if(out == 0)
	{
		trayenable = 1;
	...
	{
		out = RegGetKey(HKEY_CURRENT_USER, "Software\\Steamupload", NULL, KEY_ALL_ACCESS | KEY_WOW64_64KEY, "TrayMinimize", &outdata);
		if (out == 2)
		{
		...
		out = RegGetKey(HKEY_CURRENT_USER, "Software\\Steamupload", REG_DWORD, KEY_ALL_ACCESS | KEY_WOW64_64KEY, "TrayEnable", &outdata);
		if (out == 2)
		{
	...
	if (CsteamuploadDlg::IsWindowVisible() != 0)
	{
...
HCURSOR CsteamuploadDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CsteamuploadDlg::OnBnClickedExit()
{
	if (init)
	{
		SteamAPI_Shutdown();
	}
	CsteamuploadDlg::OnDestroy();
	exit(2);
}

void CsteamuploadDlg::OnBnClickedMinEn()
{
	DWORD indata = 0;
...
void CsteamuploadDlg::OnBnClickedTrayEn()
{
	int tren;
	DWORD indata = 0;
...
int CsteamuploadDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CsteamuploadDlg::OnDestroy()
{
	CDialog::OnDestroy();
...
BOOL CsteamuploadDlg::TrayIsVisible()
{
	return m_bTrayIconVisible;
}

void CsteamuploadDlg::TraySetIcon(HICON hIcon)
{
	ASSERT(hIcon);
...
void CsteamuploadDlg::TraySetIcon(UINT nResourceID)
{
...
void CsteamuploadDlg::TraySetIcon(LPCTSTR lpszResourceName)
{
	HICON hIcon = 0;
...
void CsteamuploadDlg::TraySetToolTip(LPCTSTR lpszToolTip)
{
...
BOOL CsteamuploadDlg::TrayUpdate()
{
	BOOL bSuccess = FALSE;
	if (m_bTrayIconVisible)
	{
...
BOOL CsteamuploadDlg::TraySetMenu(UINT nResourceID, UINT nDefaultPos)
{
	BOOL bSuccess;
...
BOOL CsteamuploadDlg::TraySetMenu(LPCTSTR lpszMenuName, UINT nDefaultPos)
{
	BOOL bSuccess;
...
BOOL CsteamuploadDlg::TraySetMenu(HMENU hMenu, UINT nDefaultPos)
{
	m_mnuTrayMenu.Attach(hMenu);
	return TRUE;
}

LRESULT CsteamuploadDlg::OnTrayNotify(WPARAM wParam, LPARAM lParam)
{
	UINT uID;
	UINT uMsg;
...
void CsteamuploadDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CsteamuploadDlg::TraySetMinimizeToTray(BOOL bMinimizeToTray)
{
	m_bMinimizeToTray = bMinimizeToTray;
}


void CsteamuploadDlg::OnTrayRButtonDown(CPoint pt)
{
...
void CsteamuploadDlg::OnTrayLButtonDown(CPoint pt)
{

}

void CsteamuploadDlg::OnTrayLButtonDblClk(CPoint pt)
{
	if (m_bMinimizeToTray)
	{
...
void CsteamuploadDlg::OnTrayRButtonDblClk(CPoint pt)
{
}

void CsteamuploadDlg::OnTrayMouseMove(CPoint pt)
{
}
void CsteamuploadDlg::OnMinimize()
{
	if(this->IsWindowVisible())
	{ 
...
PCHAR* CsteamuploadDlg::CommandLineToArgvA(PCHAR CmdLine,int* _argc)
{
	PCHAR* argv;
	PCHAR  _argv;
	ULONG   len;
...
void CsteamuploadDlg::OnBnClickedDelete()
{
...
void CsteamuploadDlg::OnBnClickedUpload()
{
...
void CsteamuploadDlg::OnBnClickedDirupload()
{
	prompt dialog(this);
	//CHAR szFilter[] = "CSV Files (*.csv)|*.csv|All Files (*.*)|*.*||";
	wchar_t szFilter[] = L"All Files (*.*)|*.*||";
...
void CsteamuploadDlg::OnBnClickedDownload()
{
	wchar_t szFilter[] = L"All Files (*.*)|*.*||";
	int mark = listfiles->GetSelectionMark();
	if(mark != -1)
...
void CsteamuploadDlg::OnBnClickedRefresh()
{
	Clearlist();
	GetFiles();
}
